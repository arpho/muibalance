...
function _clearHook(type5, fun) {
  HOOKS[type5] = HOOKS[type5].filter((h) => h !== fun);
}
...
function useOperators(type5, operators) {
  for (const [name, fn] of Object.entries(operators)) {
    ...
    assert(!currentFn || fn === currentFn, `${name} already exists for '${type5}' operators. Cannot change operator function once registered.`);
  }
  switch (type5) {
    case "accumulator":
...
//# sourceMappingURL=chunk-KQ6HDGUI.js.map
