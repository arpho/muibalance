...
function getDexieDbWithTables(databaseName, collectionName, settings, schema) {
  ...
  var state = getFromMapOrCreate(DEXIE_STATE_DB_BY_NAME, dexieDbName, () => {
    var value = (() => __async(this, null, function* () {
...
function closeDexieDb(statePromise) {
...
function getDocsInDb(internals, docIds) {
  return __async(this, null, function* () {
    var state = yield internals;
    var docsInDb = yield state.dexieTable.bulkGet(docIds);
...
function dexieQuery(instance, preparedQuery) {
  return __async(this, null, function* () {
    ...
    var rows = [];
    yield state.dexieDb.transaction("r", state.dexieTable, (dexieTx) => __async(this, null, function* () {
...
function dexieCount(instance, preparedQuery) {
  return __async(this, null, function* () {
    var state = yield instance.internals;
    var queryPlan = preparedQuery.queryPlan;
    var queryPlanFields = queryPlan.index;
...
var RxStorageInstanceDexie = function() {
  ...
  _proto.bulkWrite = function bulkWrite(documentWrites, context) {
    return __async(this, null, function* () {
      ...
      if (!shownNonPremiumLog && !(yield hasPremiumFlag())) {
      ...
      var categorized;
      yield state.dexieDb.transaction("rw", state.dexieTable, state.dexieAttachmentsTable, () => __async(this, null, function* () {
        var docsInDbMap = /* @__PURE__ */ new Map();
        var docsInDbWithInternals = yield getDocsInDb(this.internals, documentKeys);
        ...
        if (bulkPutDocs.length > 0) {
          yield state.dexieTable.bulkPut(bulkPutDocs);
        }
        var putAttachments = [];
        ...
        yield state.dexieAttachmentsTable.bulkPut(putAttachments);
        yield state.dexieAttachmentsTable.bulkDelete(categorized.attachmentsRemove.map((attachment) => attachmentObjectId(attachment.documentId, attachment.attachmentId)));
      }));
  ...
  _proto.findDocumentsById = function findDocumentsById(ids, deleted) {
    return __async(this, null, function* () {
      ...
      var state = yield this.internals;
      var ret = [];
      yield state.dexieDb.transaction("r", state.dexieTable, () => __async(this, null, function* () {
        var docsInDb = yield getDocsInDb(this.internals, ids);
  ...
  _proto.count = function count(preparedQuery) {
    return __async(this, null, function* () {
      if (preparedQuery.queryPlan.selectorSatisfiedByIndex) {
        ...
        var _result = yield dexieQuery(this, preparedQuery);
        return {
  ...
  _proto.cleanup = function cleanup(minimumDeletedTime) {
    return __async(this, null, function* () {
      ensureNotClosed(this);
      var state = yield this.internals;
      yield state.dexieDb.transaction("rw", state.dexieTable, () => __async(this, null, function* () {
        var maxDeletionTime = now() - minimumDeletedTime;
        var toRemove = yield state.dexieTable.where("_meta.lwt").below(maxDeletionTime).toArray();
        var removeIds = [];
        ...
        yield state.dexieTable.bulkDelete(removeIds);
      }));
      return true;
    });
  };
  _proto.getAttachmentData = function getAttachmentData(documentId, attachmentId, _digest) {
    return __async(this, null, function* () {
      ...
      return yield state.dexieDb.transaction("r", state.dexieAttachmentsTable, () => __async(this, null, function* () {
        var attachment = yield state.dexieAttachmentsTable.get(id);
        if (attachment) {
  ...
  _proto.close = function close() {
    ...
    this.closed = (() => __async(this, null, function* () {
      this.changes$.complete();
      yield closeDexieDb(this.internals);
    }))();
    return this.closed;
  };
...
function createDexieStorageInstance(storage, params, settings) {
  return __async(this, null, function* () {
    ...
    yield addRxStorageMultiInstanceSupport(RX_STORAGE_NAME_DEXIE, params, instance);
