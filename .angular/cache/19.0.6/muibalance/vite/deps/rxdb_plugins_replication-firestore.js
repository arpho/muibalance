...
// node_modules/rxdb/node_modules/@firebase/util/dist/index.esm2017.js
...
// node_modules/rxdb/node_modules/@firebase/component/dist/esm/index.esm2017.js
var Component = class {
  /**
   *
...
// node_modules/rxdb/node_modules/@firebase/logger/dist/esm/index.esm2017.js
var instances = [];
var LogLevel;
(function(LogLevel2) {
...
// node_modules/rxdb/node_modules/@firebase/app/dist/esm/index.esm2017.js
...
// node_modules/rxdb/node_modules/@firebase/webchannel-wrapper/dist/bloom-blob/esm/bloom_blob_es2018.js
...
// node_modules/rxdb/node_modules/@firebase/webchannel-wrapper/dist/webchannel-blob/esm/webchannel_blob_es2018.js
...
// node_modules/rxdb/node_modules/@firebase/firestore/dist/index.esm2017.js
...
function swapDefaultDeletedTodeletedField(deletedField, doc2) {
  if (deletedField === "_deleted") {
    return doc2;
  } else {
    doc2 = flatClone(doc2);
    var isDeleted = !!doc2._deleted;
    doc2[deletedField] = isDeleted;
    delete doc2._deleted;
    return doc2;
  }
}
function handlePulledDocuments(collection, deletedField, docs) {
  return docs.map((doc2) => {
    var useDoc = flatClone(doc2);
    if (deletedField !== "_deleted") {
...
var RxReplicationState = function() {
  function RxReplicationState2(replicationIdentifier, collection, deletedField, pull, push, live, retryTime, autoStart, toggleOnDocumentVisible) {
    ...
    this.metaInfoPromise = (() => __async(this, null, function* () {
      var metaInstanceCollectionName = "rx-replication-meta-" + (yield collection.database.hashFunction([this.collection.name, this.replicationIdentifier].join("-")));
  ...
  _proto.start = function start() {
    return __async(this, null, function* () {
      ...
      var metaInfo = yield this.metaInfoPromise;
      ...
      this.internalReplicationState = replicateRxStorageInstance({
        ...
        replicationHandler: {
          masterChangeStream$: this.remoteEvents$.asObservable().pipe(filter((_v) => !!this.pull), mergeMap((ev) => __async(this, null, function* () {
            if (ev === "RESYNC") {
              return ev;
            }
          ...
          masterChangesSince: (checkpoint, batchSize) => __async(this, null, function* () {
            if (!this.pull) {
              return {
                checkpoint: null,
                documents: []
            ...
            while (!done && !this.isStoppedOrPaused()) {
              try {
                result = yield this.pull.handler(checkpoint, batchSize);
                done = true;
              } catch (err) {
                ...
                yield awaitRetry(this.collection, ensureNotFalsy(this.retryTime));
              }
            }
          ...
          masterWrite: (rows) => __async(this, null, function* () {
            if (!this.push) {
              return [];
            }
            var done = false;
            ...
            var useRowsOrNull = yield Promise.all(rows.map((row) => __async(this, null, function* () {
            ...
            while (!done && !this.isStoppedOrPaused()) {
              try {
                result = yield this.push.handler(useRows);
  ...
  _proto.cancel = function cancel() {
    return __async(this, null, function* () {
      ...
      if (this.internalReplicationState) {
        yield cancelRxStorageReplication(this.internalReplicationState);
      }
  ...
  _proto.remove = function remove() {
    return __async(this, null, function* () {
      yield ensureNotFalsy(this.metaInstance).remove();
      var metaInfo = yield this.metaInfoPromise;
      yield this.cancel();
      yield removeConnectedStorageFromCollection(this.collection, metaInfo.collectionName, metaInfo.schema);
    });
  };
...
// node_modules/rxdb/dist/esm/plugins/replication-firestore/firestore-helper.js
...
// node_modules/rxdb/dist/esm/plugins/replication-firestore/index.js
var RxFirestoreReplicationState = function(_RxReplicationState) {
  function RxFirestoreReplicationState2(firestore, replicationIdentifierHash, collection, pull, push, live = true, retryTime = 1e3 * 5, autoStart = true) {
    var _this;
  ...
  _inheritsLoose(RxFirestoreReplicationState2, _RxReplicationState);
  return RxFirestoreReplicationState2;
}(RxReplicationState);
function replicateFirestore(options) {
  ...
  if (schemaPart || // also must not be nested.
  ...
  var pullFilters = options.pull?.filter !== void 0 ? toArray(options.pull.filter) : [];
    ...
    replicationPrimitivesPull = {
      handler(lastPulledCheckpoint, batchSize) {
        return __async(this, null, function* () {
          ...
          var useDocs = [];
          while (mustsReRun) {
            yield waitForPendingWrites(options.firestore.database);
            yield runTransaction(options.firestore.database, (_tx) => __async(this, null, function* () {
              useDocs = [];
              var [newerQueryResult, sameTimeQueryResult] = yield Promise.all([getDocs(newerQuery), sameTimeQuery ? getDocs(sameTimeQuery) : void 0]);
              if (newerQueryResult.metadata.hasPendingWrites || sameTimeQuery && ensureNotFalsy(sameTimeQueryResult).metadata.hasPendingWrites) {
                ...
                if (missingAmount > 0) {
                  var additionalDocs = newerQueryResult.docs.slice(0, missingAmount).filter((x2) => !!x2);
  ...
  if (options.push) {
    ...
    replicationPrimitivesPush = {
      handler(rows) {
        return __async(this, null, function* () {
          if (pushFilter !== void 0) {
            rows = yield asyncFilter(rows, (row) => pushFilter(row.newDocumentState));
          }
          var writeRowsById = {};
          ...
          yield runTransaction(options.firestore.database, (_tx) => __async(this, null, function* () {
            ...
            var getQuery = (ids) => {
              return getDocs(query(options.firestore.collection, where(documentId(), "in", ids))).then((result) => result.docs).catch((error) => {
                if (error?.code && error.code === "permission-denied") {
                  return Promise.all(ids.map((id) => getDoc(doc(options.firestore.collection, id)))).then((docs) => docs.filter((doc2) => doc2.exists()));
                }
                throw error;
              });
            };
            var docsInDbResult = yield getContentByIds(docIds, getQuery);
            var docsInDbById = {};
            ...
            var hasWrite = false;
            yield Promise.all(Object.entries(writeRowsById).map((_0) => __async(this, [_0], function* ([docId, writeRow]) {
...
@firebase/firestore/dist/index.esm2017.js:
   ...
   *)
*/
//# sourceMappingURL=rxdb_plugins_replication-firestore.js.map
